...
export function getParentDisplayName(parentProfile?: ParentMeResponse, defaultName = "Parent"): string {
...
export default function App({ dict }: { dict: any }) {
  ...
  const parentName = getParentDisplayName(parentProfile, dict.app.parent);
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">{dict.app.loading}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-slate-50">
        <div className="text-center">
          <p className="text-sm text-red-500 font-medium">{error?.message ?? dict.app.failedToLoad}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-slate-50 p-6">
        <div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm text-center">
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{dict.app.noStudentsTitle}</h1>
          <p className="mt-3 text-sm text-slate-600 font-medium">
            {dict.app.noStudentsDesc}
          </p>
        </div>
      </div>
    );
  ...
  const renderModule = () => {
    switch (activeModule) {
      case "Dashboard": return <OverviewModule child={child} setActiveModule={setActiveModule} onOpenPlanner={openPlanner} dict={dict} />;
      case "Grades": return <GradesModule child={child} setActiveModule={setActiveModule} dict={dict} />;
      case "Attendance": return <AttendanceModule child={child} dict={dict} />;
      case "Assignments": return <AssignmentsModule child={child} dict={dict} />;
      case "Gradebook": return <GradebookModule child={child} dict={dict} />;
      case "Analytics": return <AnalyticsModule child={child} dict={dict} />;
      case "Messages": return <MessagesModule child={child} activeThread={activeMessageThread} setActiveThread={setActiveMessageThread} dict={dict} />;
      case "Notifications": return <NotificationsModule child={child} dict={dict} />;
      case "Schedule": return <ScheduleModule child={child} dict={dict} />;
      default: return <OverviewModule child={child} setActiveModule={setActiveModule} onOpenPlanner={openPlanner} dict={dict} />;
    }
  };

  const navItems = [
    { icon: LayoutDashboard, label: dict.nav.dashboard, module: "Dashboard" },
    { icon: UserCheck, label: dict.nav.attendance, module: "Attendance" },
    { icon: GraduationCap, label: dict.nav.grades, module: "Grades" },
    { icon: ClipboardList, label: dict.nav.assignments, module: "Assignments", badge: badges.Assignments },
    { icon: BookOpen, label: dict.nav.gradebook, module: "Gradebook" },
    { icon: BarChart3, label: dict.nav.analytics, module: "Analytics" },
    { icon: MessageSquare, label: dict.nav.messages, module: "Messages", badge: badges.Messages },
    { icon: Bell, label: dict.nav.notifications, module: "Notifications", badge: badges.Notifications },
  ];
    ...
    <div className="h-screen w-full bg-slate-50 font-sans text-slate-800 overflow-hidden relative">
      ...
      <div className="block md:hidden h-full flex flex-col overflow-hidden relative">
        <div className="sticky top-0 bg-white border-b border-slate-100 z-30 shrink-0 select-none">
          ...
          <div className="px-4 pb-3 pt-1 border-t border-slate-50 bg-white">
            <h2 className="text-sm font-semibold text-slate-800">{dict.app.goodMorning.replace('{parentName}', parentName)}</h2>
            <p className="text-xs text-slate-400 mt-0.5 font-medium">{child.name} · {dict.app.grade} {child.grade} · {dict.app.sec} {child.section}</p>
          </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 z-40 select-none pb-[env(safe-area-inset-bottom)]">
          <div className="grid grid-cols-5 h-16 pt-1">
            {[
              { icon: LayoutDashboard, label: dict.nav.home, module: "Dashboard" },
              { icon: UserCheck, label: dict.nav.attendance, module: "Attendance" },
              { icon: ClipboardList, label: dict.nav.tasks, module: "Assignments", badge: badges.Assignments },
              { icon: MessageSquare, label: dict.nav.messages, module: "Messages", badge: badges.Messages },
            ...
            <button onClick={() => setShowMoreSheet(true)} className={`flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors relative min-h-[44px] border-none bg-transparent ${["Grades","Gradebook","Schedule","Notifications","Analytics"].includes(activeModule) ? "text-[#3949AB]" : "text-slate-400"}`}>
              ...
              <span className="text-[10px] font-bold">{dict.app.more}</span>
            </button>
          </div>
        </div>
            ...
            <div className="fixed inset-0 z-50 md:hidden">
              ...
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 250 }} className="absolute inset-x-0 bottom-0 bg-white rounded-t-3xl shadow-2xl pb-[calc(2rem+env(safe-area-inset-bottom))] border-t border-slate-100 p-6 flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-black uppercase tracking-wider text-[#3949AB]">{dict.app.moreApplications}</h3>
                ...
                <div className="grid grid-cols-5 gap-2 text-center">
                  {[
                    { icon: GraduationCap, label: dict.nav.grades, module: "Grades" },
                    { icon: BookOpen, label: dict.nav.gradebook, module: "Gradebook" },
                    { icon: Calendar, label: dict.nav.schedule, module: "Schedule" },
                    { icon: Bell, label: dict.nav.notifications, module: "Notifications", badge: badges.Notifications },
                    { icon: BarChart3, label: dict.nav.analytics, module: "Analytics" },
                  ].map(({ icon: Icon, label, module, badge }) => (
      ...
      <div className="hidden md:flex h-full w-full overflow-hidden">
        <aside className={`relative flex flex-col transition-all duration-300 ${isSidebarCollapsed ? "w-20" : "w-64"} h-full bg-white border-r border-slate-100 shrink-0`}>
          ...
          <div className={`py-5 border-b border-slate-50 flex items-center justify-between ${isSidebarCollapsed ? "px-4" : "px-6"}`}>
            <div className="flex-1">
              {!isSidebarCollapsed && <div className="flex items-center justify-between mb-2">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{dict.app.parentMode}</p>
                <LanguageSwitcher currentLang={lang} />
              </div>}
              {isSidebarCollapsed && <div className="mb-4 flex justify-center"><LanguageSwitcher currentLang={lang} /></div>}
          ...
          <div onClick={() => setIsChildModalOpen(true)} className={`cursor-pointer hover:bg-slate-100 transition-colors bg-slate-50 border border-slate-100 ${isSidebarCollapsed ? "mx-3 my-4 p-2.5 rounded-full flex justify-center" : "mx-4 my-4 p-3 rounded-xl"}`}>
            {isSidebarCollapsed ? (
              ...
              <>
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 leading-none">{dict.app.viewingChild}</p>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-slate-400">{dict.app.grade} {child.grade} · {dict.app.sec} {child.section}</p>
                  </div>
          ...
          <div className={`border-t border-slate-50 ${isSidebarCollapsed ? "p-3 flex justify-center" : "p-4"}`}>
            <div className={`flex items-center bg-slate-50/50 rounded-xl ${isSidebarCollapsed ? "p-1 justify-center" : "gap-3 p-2"}`}>
              ...
              {!isSidebarCollapsed && <div><p className="text-xs font-bold text-slate-900 uppercase tracking-tight">{parentName}</p><p className="text-[10px] font-medium text-slate-400">{dict.app.parentAccount}</p></div>}
            </div>
          </div>
        </aside>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.94, opacity: 0, y: 15 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: "spring", duration: 0.35 }} className="w-full max-w-md bg-white rounded-3xl shadow-2xl relative z-60 overflow-visible border border-slate-100/80 flex flex-col">
              <div className="pt-6 px-6 pb-4 flex items-start justify-between">
                <div><h3 className="text-xl font-bold tracking-tight text-slate-900">{dict.app.switchChild}</h3><p className="text-sm font-medium text-slate-500 mt-0.5">{dict.app.family} · {children.length} {dict.app.childrenEnrolled}</p></div>
              ...
              <div className="bg-[#faf8f5] hover:bg-[#faf5ec] border-y border-stone-100/70 py-4 px-6 flex items-center justify-between cursor-pointer transition-colors group select-none" onClick={() => { setToastMessage(dict.app.consolidatedMsg); setIsChildModalOpen(false); }}>
                <div className="flex items-center gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0"><span className="text-sm font-bold text-slate-800 flex items-center gap-1.5">{dict.app.viewAllChildren} <ArrowUpRight size={14} className="text-[#3949ab] shrink-0" /></span><p className="text-xs text-slate-500 font-medium mt-0.5">{dict.app.seeCombined}</p></div>
                </div>
                  ...
                  <div key={c.id} onClick={() => { setSelectedChildIndex(i); setIsChildModalOpen(false); }} className={`flex items-start gap-4 py-4 px-6 cursor-pointer transition-all select-none group relative ${selectedChildIndex === i ? "bg-[#f3f5ff]" : "bg-white hover:bg-slate-50/60"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1.5">
                        ...
                        {selectedChildIndex === i ? <span className="text-[11px] font-bold text-[#3949ab] shrink-0">{dict.app.viewing}</span> : <ChevronRight size={18} className="text-slate-400 group-hover:translate-x-0.5 transition-transform shrink-0" />}
                      </div>
                      <p className="text-[11px] text-slate-500 font-medium mt-0.5 truncate">{dict.app.grade} {c.grade}{c.section} · {c.subjects.map((s) => s.name).join(", ")}</p>
                    </div>
                  </div>
                ))}
              </div>
