...
export default function App() {
      ...
      <div className="h-screen w-full flex items-center justify-center bg-slate-50">
        <div className="text-center">
          <p className="text-sm text-red-500 font-medium">
            {error?.message ?? t("app.failedToLoad")}
          </p>
        </div>
      </div>
    );
  ...
  const renderModule = () => {
    switch (activeModule) {
      ...
      case "Grades":
        return <GradesModule child={child} setActiveModule={openModule} />;
      case "Attendance":
        return <AttendanceModule child={child} />;
      case "Assignments":
        return <AssignmentsModule child={child} />;
      case "Gradebook":
        return <GradebookModule child={child} />;
      case "Analytics":
        return <AnalyticsModule child={child} />;
      case "Messages":
        return (
          <MessagesModule
            child={child}
            externalThreadId={selectedMessageThreadId}
          />
        );
      case "Notifications":
        return <NotificationsModule child={child} />;
      case "Schedule":
        return <ScheduleModule child={child} />;
      default:
        return (
          <OverviewModule
            child={child}
    ...
    <ErrorBoundary>
      <div className="h-screen w-full bg-slate-50 font-sans text-slate-800 overflow-hidden relative">
          ...
          <div className="sticky top-0 bg-white border-b border-slate-100 z-30 shrink-0 select-none">
            <div className="h-14 flex items-center justify-between px-4">
              <button
                ...
                className="flex items-center gap-2 bg-[#f0f4ff] rounded-full px-3 py-1.5 max-w-[210px] cursor-pointer min-h-[38px] border-none"
              >
                <div className="w-5 h-5 rounded-full bg-[#3949AB] text-white flex items-center justify-center font-bold text-[9px] shrink-0">
                  {child.initials}
                </div>
                <span className="text-xs font-bold text-[#3949AB] truncate">
                  {child.name}
                </span>
              ...
              <button
                ...
                className="w-11 h-11 rounded-full hover:bg-slate-50 flex items-center justify-center text-slate-500 relative cursor-pointer border-none bg-transparent"
              >
                <Bell size={20} />
            ...
            <div className="px-4 pb-3 pt-1 border-t border-slate-50 bg-white">
              <h2 className="text-sm font-semibold text-slate-800">
                {t("app.goodMorning", { parentName })}
              </h2>
              <p className="text-xs text-slate-400 mt-0.5 font-medium">
                {child.name} · {t("app.grade")} {child.grade} · {t("app.sec")}{" "}
                {child.section}
              </p>
            </div>
          </div>
          ...
          <div
            className={
              activeModule === "Messages"
                ? "flex-1 flex flex-col overflow-hidden bg-white animate-fade-in pb-16"
                : "flex-1 overflow-y-auto px-4 pt-4 pb-24 custom-scrollbar bg-slate-50 animate-fade-in"
            }
          >
            {["Messages", "Analytics"].includes(activeModule) ? (
              renderModule()
            ) : (
              <div className="max-w-md mx-auto">{renderModule()}</div>
            )}
          </div>
            ...
            <div className="grid grid-cols-5 h-16 pt-1">
              {[
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors min-h-[44px] border-none bg-transparent ${activeModule === module ? "text-[#3949AB]" : "text-slate-400"}`}
                >
                  <div className="relative">
                    <Icon
                      size={20}
                      strokeWidth={activeModule === module ? 2.5 : 2}
                    />
                    {badge !== undefined && badge > 0 && (
                      <span className="absolute -top-1.5 -right-2 bg-rose-500 text-white font-black text-[8px] h-4 min-w-4 px-1 rounded-full flex items-center justify-center ring-2 ring-white">
                        {badge}
                      </span>
                    )}
                  </div>
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors relative min-h-[44px] border-none bg-transparent ${["Grades", "Gradebook", "Schedule", "Notifications", "Analytics"].includes(activeModule) ? "text-[#3949AB]" : "text-slate-400"}`}
              >
                <div className="relative">
                  <Menu size={20} />
                  {badges.Notifications > 0 && (
                    <span className="absolute -top-1 -right-1 bg-rose-500 w-2 h-2 rounded-full ring-2 ring-white" />
                  )}
                </div>
            ...
            {showMoreSheet && (
              <div className="fixed inset-0 z-50 md:hidden">
                ...
                <motion.div
                  ...
                  className="absolute inset-x-0 bottom-0 bg-white rounded-t-3xl shadow-2xl pb-[calc(2rem+env(safe-area-inset-bottom))] border-t border-slate-100 p-6 flex flex-col gap-6"
                >
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-black uppercase tracking-wider text-[#3949AB]">
                      {t("app.moreApplications")}
                    </h3>
                    <button
                      onClick={() => setShowMoreSheet(false)}
                      className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 border-none"
                    >
                      ✕
                    </button>
                  </div>
                  <div className="grid grid-cols-5 gap-2 text-center">
                    {[
                      ...
                      <button
                        ...
                        className="flex flex-col items-center gap-2 cursor-pointer group min-h-[60px] border-none bg-transparent"
                      >
                        <div
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all relative ${activeModule === module ? "bg-[#3949AB] text-white shadow-lg" : "bg-slate-50 text-slate-600 group-hover:bg-slate-100"}`}
                        >
                          <Icon size={22} />
                          {badge !== undefined && badge > 0 && (
                            <span className="absolute top-0 right-0 bg-rose-500 text-white font-black text-[8px] h-4 min-w-4 px-1 rounded-full flex items-center justify-center ring-2 ring-white">
                              {badge}
                            </span>
                          )}
                        </div>
                  ...
                  <div className="border-t border-slate-100 pt-4 space-y-1">
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 cursor-pointer border-none bg-transparent text-left"
                    >
                      <div className="w-9 h-9 rounded-full bg-[#3949AB] flex items-center justify-center text-white font-bold text-xs">
                        {parentInitials}
                      </div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-rose-50 cursor-pointer border-none bg-transparent text-left"
                    >
        ...
        <div className="hidden md:flex h-full w-full overflow-hidden">
          <aside
            className={`relative flex flex-col transition-all duration-300 ${isSidebarCollapsed ? "w-20" : "w-64"} h-full bg-white border-r border-slate-100 shrink-0`}
          >
            <button
              onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
              className="absolute -right-3 top-8 z-40 w-6 h-6 rounded-full bg-white border border-slate-200/85 shadow-md flex items-center justify-center text-slate-500 hover:text-slate-800 transition-all cursor-pointer"
              type="button"
            >
              {isSidebarCollapsed ? (
            ...
            <div
              className={`border-b border-slate-100 bg-linear-to-b from-white to-slate-50/30 ${isSidebarCollapsed ? "px-4 pt-5 pb-4 flex justify-center" : "px-4 sm:px-5 pt-5 sm:pt-7 pb-4"}`}
            >
              <div
            ...
            <div
              onClick={() => setIsChildModalOpen(true)}
              className={`cursor-pointer hover:bg-slate-100 transition-colors bg-slate-50 border border-slate-100 ${isSidebarCollapsed ? "mx-3 my-4 p-2.5 rounded-full flex justify-center" : "mx-4 my-4 p-3 rounded-xl"}`}
            >
              {isSidebarCollapsed ? (
                ...
                <>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 leading-none">
                    {t("app.viewingChild")}
                  </p>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[9px] text-slate-400">
                        {t("app.grade")} {child.grade} · {t("app.sec")}{" "}
                        {child.section}
                      </p>
                    </div>
                    <ChevronDown
            ...
            <nav
              className={`flex-1 flex flex-col gap-1 overflow-y-auto ${isSidebarCollapsed ? "px-2 py-4 items-center" : "px-4 py-4"}`}
            >
            ...
            <div
              className={`border-t border-slate-50 ${isSidebarCollapsed ? "p-3 flex justify-center" : "p-4"}`}
              ref={profileMenuRef}
            >
              <button
                type="button"
              ...
              >
                <div className="w-10 h-10 rounded-full bg-[#3949AB] flex items-center justify-center text-white font-bold text-sm shadow-sm ring-2 ring-white shrink-0">
                  {parentInitials}
                </div>
                {!isSidebarCollapsed && (
                  <>
              ...
              {isProfileMenuOpen && (
                <div
                  className={`mt-2 bg-white border border-slate-100 rounded-xl shadow-lg overflow-hidden ${isSidebarCollapsed ? "absolute bottom-16 left-3 right-3" : ""}`}
                >
                  <button
                    type="button"
                    onClick={() => {
          ...
          <main className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-hidden relative">
            ...
            >
              {activeModule === "Messages" ? (
                ...
                <div className="max-w-7xl mx-auto pb-12">{renderModule()}</div>
              )}
            </div>
          </main>
        </div>
          ...
          {isChildModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="w-full max-w-md bg-white rounded-3xl shadow-2xl relative z-60 overflow-visible border border-slate-100/80 flex flex-col"
              >
                <div className="pt-6 px-6 pb-4 flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-500 mt-0.5">
                      {t("app.family")} · {children.length}{" "}
                      {t("app.childrenEnrolled")}
                    </p>
                  </div>
                  <button
                    onClick={() => setIsChildModalOpen(false)}
                    className="w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200/80 text-slate-500 flex items-center justify-center transition-all cursor-pointer border-none"
                  >
                ...
                <div className="divide-y divide-slate-100 overflow-hidden rounded-b-3xl">
                  {children.map((c: Child, i: number) => (
                    <div
                      ...
                      className={`flex items-start gap-4 py-4 px-6 cursor-pointer transition-all select-none group relative ${selectedChildIndex === i ? "bg-[#f3f5ff]" : "bg-white hover:bg-slate-50/60"}`}
                    >
                      <div
                        className={`w-12 h-12 rounded-full ${avatarColors[i]} text-white flex items-center justify-center font-bold text-sm tracking-wide shrink-0 shadow-xs ring-2 ring-white`}
                      >
                        {c.initials}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1.5">
                          ...
                          {selectedChildIndex === i ? (
                            <span className="text-[11px] font-bold text-[#3949ab] shrink-0">
                              {t("app.viewing")}
                            </span>
                          ) : (
                            <ChevronRight
                ...
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 z-20">
                  <button
                    onClick={() => setIsChildModalOpen(false)}
                    className="w-9 h-9 rounded-full bg-white border border-slate-200 shadow-md flex items-center justify-center text-slate-600 cursor-pointer"
                  >
        ...
        <PlannerModule
          ...
          organizationId={
            parentProfile?.student_details.find(
              (student) => student.id === child.id,
            )?.organization ?? parentProfile?.organization_ids[0]
          }
