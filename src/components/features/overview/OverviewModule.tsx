...
export const OverviewModule = ({
  ...
  const { data: chatThreads = [], isLoading: isThreadsLoading } = useQuery({
  ...
  const unreadTeacherThreads = React.useMemo(() => {
    const teacherAssignmentByTeacherId = new Map(
      teacherAssignments.map(
        (assignment) => [assignment.teacher_id, assignment] as const,
      ),
    );

    return chatThreads
      .filter(
        (thread) => thread.student === child.id && thread.unread_count > 0,
      )
      .sort(
        (left, right) =>
      ...
      .map((thread, index) => {
        const assignment =
          teacherAssignmentByTeacherId.get(thread.teacher) ?? null;
        const latestMessage = thread.latest_message;
        const teacherName = assignment?.teacher_name || "Teacher";
        return {
          id: thread.id,
          teacherName,
          teacherInitials: toInitials(teacherName),
          subjectName: assignment?.subject_name || "Conversation",
          preview:
          ...
          avatarBg: ["bg-[#3949AB]", "bg-[#128267]", "bg-[#c85a23]"][index % 3],
        };
      });
  }, [chatThreads, child.id, teacherAssignments]);
  const isMessagesLoading =
    isThreadsLoading ||
    (Boolean(child.sectionId) && isTeacherAssignmentsLoading);
              ...
              <div className="space-y-3.5 my-4">
                ...
                {!isHomeworkLoading &&
                  ...
                  todaysHomework.map((hw) => {
                    const isConfirmed =
                      hw.confirmed || hw.homeworkConfirmation?.is_confirmed;
                    ...
                    return (
                      ...
                      >
                        <div className="w-2.5 h-2.5 rounded-full mt-1.5 shrink-0 bg-[#3949ab]" />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="shrink-0">
                              <Badge
                                variant={getHomeworkBadgeVariant({
          ...
          <Card className="h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4">
                <SectionLabel>
                  {t("overview.messagesFromTeachers")}
                </SectionLabel>
                <button
              ...
              <div className="divide-y divide-slate-50">
                {isMessagesLoading && (
                  <div className="py-4 text-sm text-slate-400">
                    Loading teacher messages...
                  </div>
                )}

                {!isMessagesLoading &&
                  unreadTeacherThreads.map((thread) => (
                    <button
                      key={thread.id}
                      type="button"
                      onClick={() => {
                        if (onOpenMessageThread) {
                          onOpenMessageThread(thread.id);
                          return;
                        }
                        setActiveModule("Messages");
                      }}
                      className="w-full py-3 flex items-center gap-3 cursor-pointer hover:bg-slate-50 rounded-lg px-2 -mx-2 transition-colors text-left"
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 ${thread.avatarBg}`}
                      >
                ...
                {!isMessagesLoading && unreadTeacherThreads.length === 0 && (
                  <div className="py-4 text-sm text-slate-400">
          ...
          <div className="relative z-10 w-full sm:max-w-lg bg-slate-50 rounded-t-[1.75rem] sm:rounded-3xl border border-slate-200/80 shadow-2xl overflow-hidden max-h-[92vh] sm:max-h-[88vh]">
            ...
            <div className="overflow-y-auto px-4 py-4 sm:px-6 sm:py-5 space-y-4">
              <div className="flex items-center justify-between gap-3">
                <Badge
                  variant={getHomeworkBadgeVariant({
                    confirmed: selectedHomeworkIsConfirmed,
                  })}
                >
                  {getHomeworkBadgeText({
                    confirmed: selectedHomeworkIsConfirmed,
                ...
                <p className="text-[11px] font-semibold text-slate-400">
                  {t("overview.due", {
                    date: formatDueDate(selectedHomework.dueDate),
                  })}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="rounded-2xl border border-slate-200/70 bg-white p-3">
                  ...
                  <p className="text-sm font-semibold text-slate-800 mt-2">
                    {selectedHomework.studentRollNo ||
              ...
              <div className="rounded-2xl border border-slate-200/70 bg-white p-4">
                ...
                <p className="text-sm leading-6 text-slate-700 mt-3">
                  {selectedHomework.description?.trim() ||
            ...
            <div className="bg-white px-4 py-4 sm:px-6 border-t border-slate-200/70 flex items-center justify-end gap-3">
              ...
              <button
                type="button"
                disabled={
                  selectedHomeworkIsConfirmed || selectedHomeworkIsPending
                }
                onClick={() => {
                  if (selectedHomeworkIsConfirmed || selectedHomeworkIsPending)
                    return;
                  confirmHomeworkMutation.mutate({
